...
//		TODO error de validacion
	...
	public List<Empleado> buscarDocentes (String apellido, String nombre, String cuil,
			String dni, String materia, String curso, String division, boolean estado) {
		List <Empleado> empleadosEncontrados = EmpleadoSRV.buscarEmpleados(apellido, nombre, cuil, dni, materia, curso, division, estado);
//		for(Empleado emp : empleados){
//			if(emp.sosDocente(apellido, nombre, cuil, dni, materia, curso, division, estado)) 
...
//		}
	...
	public void altaEmpleado (String nombre, String apellido, String dni, String cuil, Date fechaDeNacimiento, int antiguedad, int nroEscuela){
		Empleado empleado = buscarEmpleado(dni);
		if(empleado == null){
			empleado = new ConCargo();
			/**TODO No estoy seguro de que este dando bien el alta de un empleado **/
			...
			/**TODO  ver de agregar el nombre correspondiente al numero de escuela**/
			Escuela escuela = buscarEscuela(nroEscuela);
			empleado.setEscuela(escuela);
			//Pongo el estado siempre en falso, cuando se le asigna una clase o un cargo se lo pone en verdadero
			empleado.setEstado(false);
			EmpleadoSRV.grabarEmpleado(empleado);
		}
	}
	
	public void modificarEmpleado (String nombre, String apellido, String dni, String cuil, Date fechaDeNacimiento, int antiguedad, int nroEscuela, boolean estado){
		/**TODO Había pensado un do while para que se ejecute y si alguno es nulo que vuelva tire el error, pero los int y los boolean no aceptan nulos. IDEAS?*/
			Empleado empleado = buscarEmpleado(dni);
			if(empleado != null){
				empleado.setNombre(nombre);
				empleado.setApellido(apellido);
	...
	//No se si esto esta bien, fue lo que se me ocurrio para mostrar los datos que pide la GUI013
	...
	public float calcularSueldo (String dni, int mes){
		...
		float sueldoTotal = 0;
		if(empleadoS != null){
			SueldoV detalleSueldo = empleadoS.vistaSueldoEmpleado(mes); /**TODO Esto esta bien? Se hace acá el para mostrar?**/
			sueldoTotal = empleadoS.calcularSueldo();
			return sueldoTotal;
		}
		if(empleadoC != null){
			SueldoV detalleSueldo = empleadoC.vistaSueldoEmpleado(mes); /**TODO Esto esta bien? Se hace acá el para mostrar?**/
	...
	public float calcularDescuento (String dni, Date fechaActual){
		...
		if(emp != null){
			for(Novedad novedad : emp.getNovedades()){
				if(novedad.getFecha().getMonth() == fechaActual.getMonth()) //como carajo hago esto!?
...
//		}
	...
	public void modificarLicencia(String codigo, String tipo, String motivo, float haberes, int tiempo, int antiguedadRequerida, boolean certificado){
		/**TODO Había pensado un do while para que se ejecute y si alguno es nulo que vuelva tire el error, pero los int y los boolean no aceptan nulos. IDEAS?**/
	...
	public void altaLicencia(String codigo, String tipo, String motivo, float haberes, int tiempo, int antiguedadRequerida, boolean certificado){
		Licencia lic = buscarLicencia(codigo);
		if(lic == null){
...
//			if(cargo.getNombre().contains(nombreCargo) || cargo.getSueldoBasico()==sueldoBasico || cargo.getHorasTrabajo()==horasTrabajo){
//				cargos.add(cargo);
//			}
//		}
...
//		}
	...
	public void modificarClase (int numero, String nombre, String curso, String division, float valor_hc, int hsSemanales){
	...
	//Se concidera Licencia siempre y cuando sea justificable (Ver tabla de licencias)
	public void cargarLicenciaEmpleado(String dni, String codigo, String tipo, String motivo, float haberes, int tiempo, int antiguedadRequerida, boolean certificado, 
			int semanasMesCorriente, float oblig_hc_mes, float oblig_hc_noTrab, int cantClasesAusente){
		Empleado emp = buscarEmpleado(dni);
		/**TODO Hay que guardar las modificaciones que se hacan en este metodo en la BD**/
		if(emp != null){
			Date fecha = new Date();
			for(Licencia licencia : licencias){
				...
				if(codigo.equals(licencia.getCodigo())){
					for(LicenciaEmpleado licEmp : licenciasEmpleado){
						//Me fijo si el empleado ya tiene registro de esa licencia, si tiene chequeo si le quedan dias disponibles
						if(codigo.equals(licEmp.getLicencia().getCodigo())){
							//chequeo si le quedan dias disponibles
							if(licEmp.getCantDisponible()>=tiempo){
								for(Novedad novedad : emp.getNovedades()){
									if(codigo.equals(novedad.getLicencia().getCodigo())){
										novedad.setFecha(fecha);
										novedad.setLicencia(licencia);
										//Seteo la clase?? Yo la volaría de Novedad
									}
								}
								//Le descuento los dias que se esta tomando
								licEmp.setCantDisponible(licEmp.getCantDisponible()-tiempo);
								//Si tiene certificado se marca como true
								if(certificado == true)
									licEmp.getLicencia().setCertificado(certificado);
							}else{
								/**TODO El empleado no tiene la antiguedad minima requerida para la licencia solicitada, ingresar como inasistencia*/
								//Si no le alcanzan los días disponibles se incrementa el valor de cantClasesAusente de Novedad en lo pasado por 
								//parámetro menos los días que tenga disponibles y se agregan los demas a la licencia
								
								//Pongo en cero los días que le quedan porque el tiempo que quiere agregar supera a los que tiene
								...
								for(Novedad novedad : emp.getNovedades()){
									if(codigo.equals(novedad.getLicencia().getCodigo())){
										novedad.setFecha(fecha);
										novedad.setLicencia(licencia);
										//Incremento oblig_hc_noTrab en los días que no pudo cubrir la licencia
										novedad.setOblig_hc_noTrab(novedad.getOblig_hc_noTrab() + (tiempo-licEmp.getCantDisponible())); 
									}
								}
								
							}
						//Si no tiene esa licencia, chequeo la antiguedad y si puede se agrega -> Creo una nueva Novedad	
							...
							Novedad novedad = new Novedad(fecha, licencia, oblig_hc_mes, oblig_hc_noTrab, semanasMesCorriente, cantClasesAusente);
							emp.getNovedades().add(novedad);
							}else{
								/**TODO El empleado no tiene la antiguedad minima requerida para la licencia solicitada, ingresar como inasistencia*/
								//Si no tiene la antiguedad mínima requerida se incrementa el valor de cantClasesAusente de Novedad en lo pasado por parámetro
				...
				/**TODO El codigo ingresado no pertenece a una licencia existente*/
				}
			}
		}else{
...
//		return novedadesAux;
	...
	//Se considera Inasistencia cuando no es justificable y hay que efectuar el descuento correspondiente (el descuento se efectúa en calcularSueldo() )
	public void cargarInasistenciaEmpleado(String dni, Date fecha, Licencia licencia, int semanasMesCorriente, float oblig_hc_mes, float oblig_hc_noTrab, int cantClasesAusente){
	...
	public List<Empleado> buscarEmpleados(String nombre, String apellido, String cuil, String dni,
			boolean estado, Escuela escuela) {
